...
App routing means letting CC Gateway route a specific application's API requests through the local routing service.
...
When routing is enabled, CC Gateway modifies the app's configuration file to point the API endpoint to the local routing service.
...
1. Identifies the request source (Claude/Codex/Grok Build)
...
When disabling routing, CC Gateway will:
...
Before enabling routing, CC Gateway backs up the original configuration and restores it when disabled.
