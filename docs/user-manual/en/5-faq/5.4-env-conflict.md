...
CC Gateway automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with CC Gateway configuration
[Expand] [Dismiss]
```
...
3. CC Gateway will automatically back up and remove the selected variables

### Automatic Backup

A backup is automatically created before removal:

- Backup location: `~/.cc-gateway/env-backups/`
...
If you prefer not to use CC Gateway to remove variables, you can handle them manually:
...
- CC Gateway provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use CC Gateway to manage configurations**: Avoid setting API keys in system environment variables
...
1. Find the backup file: `~/.cc-gateway/env-backups/`
