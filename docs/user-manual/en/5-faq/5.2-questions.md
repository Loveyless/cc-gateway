...
The maintenance fork currently has no published macOS signing or notarization guarantee. Verify release assets and signatures before installing a build from the [Releases page](https://github.com/Loveyless/cc-gateway/releases).
...
2. Add CC Gateway to your antivirus software's whitelist
...
chmod +x CC-Gateway-*.AppImage

# If it still fails, try
./CC-Gateway-*.AppImage --no-sandbox
```
...
**Solution**: Use the dedicated `CC_GATEWAY_GDK_BACKEND` environment variable to switch back to native Wayland (it is read before GTK init, and the hook never overrides it):

```bash
CC_GATEWAY_GDK_BACKEND=wayland ./CC-Gateway-*.AppImage
```

- When launching from a desktop icon, add it to the `.desktop` `Exec=` line (e.g. `env CC_GATEWAY_GDK_BACKEND=wayland /path/to/AppImage`) or set it in your session environment — otherwise an icon launch won't see the variable.
- The variable is generic: on tiling Wayland compositors (sway/Hyprland) where clicks don't register, set `CC_GATEWAY_GDK_BACKEND=x11` instead.
...
**Solutions**:
1. Check if the `~/.cc-gateway/` directory exists
2. Restore from backup: `~/.cc-gateway/backups/`
...
**Solutions**:
1. Confirm the file was exported by CC Gateway
...
**Windows**:
- Check taskbar settings to ensure the CC Gateway icon is not hidden
...
2. Restart the app
3. Delete `~/.cc-gateway/settings.json` to reset settings
...
3. Download and install the latest CC Gateway build manually from this project's Releases page (this fork does not publish a Homebrew cask yet)

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and CC Gateway runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, CC Gateway covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `ccgateway://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/Loveyless/cc-gateway/issues)
...
CC Gateway stores logs under its application configuration directory. By default this is `.cc-gateway` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.

When submitting an Issue, attach the files relevant to the problem:

- General, network, or proxy errors: `~/.cc-gateway/logs/cc-gateway.log` and its rotated files
- Application crashes: `~/.cc-gateway/crash.log`, `crash.log.1`, and `crash.log.2`
- The default Windows location is `C:\Users\<username>\.cc-gateway\...`
