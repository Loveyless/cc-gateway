...
> **Current boundary**: Local import, per-app toggles, and projection only. GitHub discovery, custom repos, ZIP, and skills.sh are retired. Marketplace install steps below are historical.
...
CC Gateway comes pre-configured with the following GitHub repositories:
...
CC Gateway provides powerful search and filter features:
...
| Codex | `~/.codex/skills/` |
| Grok Build | Grok Build skills directory |
...
- **Automatic backup**: Before deletion, the skill is backed up to `~/.cc-gateway/skill-backups/`
- Removes the skill from all app directories (Claude, Codex, Gemini, OpenCode, Hermes)
- Removes the skill from the SSOT directory (`~/.cc-gateway/skills/`)
...
CC Gateway compares installed skills with the remote repository version using **SHA-256 content hashes**. Whenever the remote has any content changes, the corresponding local skill card automatically shows an "Update available" indicator.
...
2. CC Gateway batch-downloads all skills with pending updates
...
| ------------------------ | --------------------------------------------------------------------- |
| **CC Gateway built-in**   | Default location `~/.cc-gateway/skills/`, managed by CC Gateway         |
| **`~/.agents/skills`**   | A shared directory conforming to community agent tool conventions, better for cross-tool collaboration |

### How to Switch

Select the target storage location from the settings or management menu in the Skills panel. The switch **does not lose skill state** — CC Gateway smoothly migrates existing skills to the new location.
...
v3.13.0 integrates **skills.sh** public registry search so you can discover community skills directly inside CC Gateway.
...
- CC Gateway has not finished the latest scan
