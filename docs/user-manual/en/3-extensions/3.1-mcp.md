...
> **Note**: Claude Desktop does not currently support CC Gateway MCP sync. MCP is supported for Claude, Codex, and Grok Build.

### Toggle Implementation

When enabling an app's toggle, CC Gateway will:

1. **Update database**: Set the server's `apps.claude/codex/grokbuild` status to `true`
...
2. Select the app to import from (Claude/Codex/Grok Build)
