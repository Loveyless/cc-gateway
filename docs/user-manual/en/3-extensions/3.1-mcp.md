...
| Grok Build | Sync to Grok Build | MCP config under the Grok Build directory |
| Hermes | Sync to Hermes | `~/.hermes/config.yaml`'s `mcp_servers` |

> **Note**: Claude Desktop does not currently support CC Gateway MCP sync. MCP is supported for Claude, Codex, Grok Build, and Hermes.

### Toggle Implementation

When enabling an app's toggle, CC Gateway will:

1. **Update database**: Set the server's `apps.claude/codex/grokbuild/hermes` status to `true`
...
When disabling an app's toggle, CC Gateway will:
...
- **Grok Build**: Requires the Grok Build config directory to exist
...
If you have already configured MCP servers in CLI tools, you can import them into CC Gateway:

1. Click the "Import" button
2. Select the app to import from (Claude/Codex/Grok Build/Hermes)
3. CC Gateway reads the existing configuration and imports it
