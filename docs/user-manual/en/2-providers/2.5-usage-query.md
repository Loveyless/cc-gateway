...
> **Retired**: This maintenance build no longer ships subscription quota, balance scripts, or Usage Script. Use [4.4 Usage Statistics](../4-proxy/4.4-usage.md) for proxy request logs. The rest of this page is historical.

CC Gateway's quota / balance display is split into two categories: **Auto Query** (official subscription types, works out of the box) and **Manual Enable** (built-in templates + custom scripts, requires user configuration before showing).

| Category                       | Scope                                                                      | User Enable Required |
| ------------------------------ | -------------------------------------------------------------------------- | -------------------- |
| **Auto Query**                 | Claude / Codex / Grok Build official subscriptions, GitHub Copilot, Codex OAuth reverse proxy | No (enabled by default) |
...
| ---------------- | ----------------------------------------------------- | ----------------------------------------- |
| Official subscriptions | Claude / Codex / Grok Build official login          | Official subscription quota               |
...
These three share the common trait that **their data source is unique and semantically unambiguous** (the usage rate of an official subscription), so CC Gateway directly calls the corresponding official or OAuth query endpoint.
...
One important reason: **the same request URL (same vendor) may expose multiple query modes** — for example, both plan-based quota queries and account-level balance queries. CC Gateway cannot automatically infer which one you want, so the built-in query for such providers is **disabled by default**, leaving you to pick the right template.
...
CC Gateway provides three preset templates:
...
**Check**:
1. Confirm the provider is an official subscription type — Claude / Codex / Grok Build official login, GitHub Copilot, or Codex OAuth reverse proxy
