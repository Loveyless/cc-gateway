...
This section describes how to configure CC Gateway according to your preferences.
...
CC Gateway supports four languages:
...
When enabled, CC Gateway automatically runs when the system starts.
...
Starting from v3.13.0, CC Gateway adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger CC Gateway via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If CC Gateway runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, CC Gateway automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Configurable apps: Claude, Claude Desktop, Codex, Grok Build, Hermes.
...
Choose the terminal application that CC Gateway uses when opening a terminal.
...
The storage location for CC Gateway's own data, defaulting to `~/.cc-gateway/`.
...
The exported file name format is `cc-gateway-export-{timestamp}.sql`.
...
Configure CC Gateway's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, CC Gateway automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `cc-gateway-sync`) |
...
- CC Gateway automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, CC Gateway displays details about the remote snapshot:
- Protocol version (v2)
...
There is no local callback port. Authentication and model requests use CC Gateway's global outbound proxy configuration.

The xAI integration does **not** use an OAuth application registered to CC Gateway. It reuses the public OAuth client identity and scopes registered for the official Grok CLI (`client_id: b1a00492-073a-47ea-816f-4c329264a828`, including `grok-cli:access`) to obtain account-backed API access.
...
Displays the current CC Gateway version number, with support for:
