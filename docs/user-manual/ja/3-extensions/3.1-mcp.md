...
> **注意**：Claude Desktop は現在 CC Gateway MCP 同期に対応していません。MCP は Claude、Codex、Grok Build に対応しています。

### スイッチの動作

あるアプリのスイッチをオンにすると、CC Gateway は以下を実行します：

1. **データベースの更新**：サーバーの `apps.claude/codex/grokbuild` のステータスを `true` に設定
...
1. 「インポート」ボタンをクリック
2. インポートするアプリを選択（Claude/Codex/Grok Build）
