...
> ⚠️ **注意**：Claude Desktop 暂不支持 CC Gateway MCP 同步。MCP 功能支持 Claude、Codex 和 Grok Build。

### 开关实现机制

当开启某个应用的开关时，CC Gateway 会：

1. **更新数据库**：将服务器的 `apps.claude/codex/grokbuild` 状态设为 `true`
...
1. 点击「导入」按钮
2. 选择要导入的应用（Claude/Codex/Grok Build）
