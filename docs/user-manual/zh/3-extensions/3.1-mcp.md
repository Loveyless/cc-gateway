...
| Grok Build | 同步到 Grok Build | Grok Build 配置目录下的 MCP 配置 |
| Hermes | 同步到 Hermes | `~/.hermes/config.yaml` 的 `mcp_servers` |

> ⚠️ **注意**：Claude Desktop 暂不支持 CC Gateway MCP 同步。MCP 功能支持 Claude、Codex、Grok Build 和 Hermes。

### 开关实现机制

当开启某个应用的开关时，CC Gateway 会：

1. **更新数据库**：将服务器的 `apps.claude/codex/grokbuild/hermes` 状态设为 `true`
2. **同步到 Live 配置**：将服务器配置写入对应应用的配置文件
3. **即时生效**：下次启动 CLI 工具时自动加载新的 MCP 服务器

当关闭某个应用的开关时，CC Gateway 会：
...
- **Codex**：需存在 `~/.codex/` 目录
- **Grok Build**：需存在 Grok Build 配置目录
...
如果你已经在 CLI 工具中配置了 MCP 服务器，可以导入到 CC Gateway：

1. 点击「导入」按钮
2. 选择要导入的应用（Claude/Codex/Grok Build/Hermes）
3. CC Gateway 会读取现有配置并导入
