...
> **已下线**：当前维护版不再读写 `~/.hermes`。`hermesConfigDir` 仅作历史设置字段保留。下文仅作历史说明。
...
Hermes 使用 YAML 配置。CC Gateway 将 MCP 服务器写入 `mcp_servers`，将可编辑的供应商条目写入 `custom_providers`，读取 Hermes `providers` 字典中的只读条目，并在切换供应商时更新 `model.provider` / `model.default`。

## 配置优先级

CC Gateway 修改配置时的优先级：

1. **CC Gateway 数据库** - 单一事实源 (SSOT)
...
- CLI 工具的配置文件（会被 CC Gateway 回填）
- CC Gateway 的 `settings.json`
...
CC Gateway v3.7.0 从 JSON 文件迁移到 SQLite：
