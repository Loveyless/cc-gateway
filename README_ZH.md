...
### 🌐 上游官方网站（非本维护版）：**[ccswitch.io](https://ccswitch.io)**

> 本仓库是独立维护的 `cc-gateway` 版本，不代表上游官方；本维护版不使用上游的网站、更新器或发布文件。
...
**CC Gateway** 为你提供一个桌面应用来管理所有支持的 AI 工具。无需手动编辑配置文件，你将获得一个可视化界面，一键将供应商导入应用，一键在不同的供应商之间进行切换，内置 50+ 供应商预设、统一的 MCP, SKILLS 管理以及系统托盘即时切换功能——所有操作都基于可靠的 SQLite 数据库和原子写入机制，保护你的配置不被损坏。
...
- **Deep Link** (`ccgateway://`) — 通过 URL 一键导入供应商、MCP 服务器、提示词和技能
- 深色 / 浅色 / 跟随系统主题、开机自启、手动检查 Release 更新、原子写入、自动备份、国际化（简中/繁中/英/日）
...
CC Gateway 支持八个工具：**Claude Code**、**Claude Desktop**、**Codex**、**Gemini CLI**、**Grok Build**、**OpenCode**、**OpenClaw** 和 **Hermes**。每个工具都有专属的供应商预设和配置管理。
...
CC Gateway 使用“通用配置片段”功能，在不同的供应商之间传递 Key 和请求地址之外的通用数据，您可以在“编辑供应商”菜单的“通用配置面板”里，点击“从当前供应商提取”，把所有的通用数据提取到通用配置中，之后在新建“供应商”的时候，只要勾选“应用通用配置”（默认勾选），就会把插件等数据写入到新的供应商配置中。您的所有配置项都会保存在运行本软件的时候，第一次导入的默认供应商里面，不会丢失。
...
上游 CC Switch 的 macOS 构建可能已通过 Apple 代码签名和公证。本维护版目前没有已发布的签名/公证保证，安装前请核对 Release 资产和签名状态。
...
- **数据库**：`~/.cc-gateway/cc-gateway.db`（SQLite — 供应商、MCP、提示词、技能）
- **本地设置**：`~/.cc-gateway/settings.json`（设备级 UI 偏好设置）
- **备份**：`~/.cc-gateway/backups/`（自动轮换，保留最近 10 个）
- **SKILLS**：`~/.cc-gateway/skills/`（默认通过软链接连接到对应应用）
- **技能备份**：`~/.cc-gateway/skill-backups/`（卸载前自动创建，保留最近 20 个）
...
如果你是从桌面图标启动的，请把它写进 `.desktop` 的 `Exec=` 行（如 `env CC_GATEWAY_GDK_BACKEND=wayland /path/to/AppImage`），或在会话环境中设置。该变量是通用的：在 tiling Wayland 合成器（sway/Hyprland）下若出现点击失效，可反过来设 `CC_GATEWAY_GDK_BACKEND=x11`。不设置则保持默认行为。
...
从 [Releases](https://github.com/Loveyless/cc-gateway/releases) 页面下载最新版本的 `CC-Gateway-v{版本号}-Windows.msi` 安装包或 `CC-Gateway-v{版本号}-Windows-Portable.zip` 绿色版。

### macOS 用户

**手动下载**

从 [Releases](https://github.com/Loveyless/cc-gateway/releases) 页面下载 `CC-Gateway-v{版本号}-macOS.dmg`（推荐）或 `.zip`。

> **注意**：本维护版目前没有已发布的 macOS 签名/公证保证，打开下载的构建前请先核对 Release 说明和资产签名。

### Arch Linux 用户

目前尚未发布 CC Gateway 自有的 AUR 包。请先使用便携版 AppImage，或在本地构建，等待维护版包进入 AUR。

### Linux 用户

从 [Releases](https://github.com/Loveyless/cc-gateway/releases) 页面下载最新版本的 Linux 安装包：

- `CC-Gateway-v{版本号}-Linux.deb`（Debian/Ubuntu）
- `CC-Gateway-v{版本号}-Linux.rpm`（Fedora/RHEL/openSUSE）
- `CC-Gateway-v{版本号}-Linux.AppImage`（通用）
...
- **SSOT**（单一事实源）：所有数据存储在 `~/.cc-gateway/cc-gateway.db`（SQLite）
