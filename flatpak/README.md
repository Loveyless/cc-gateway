...
This directory contains the Flatpak manifest (`io.github.loveyless.ccgateway`) for CC Gateway, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
```bash
cp "$(find src-tauri/target/release/bundle -name '*.deb' | head -n 1)" flatpak/cc-gateway.deb
```

3) Build the local Flatpak repository and export the `.flatpak`:

```bash
flatpak-builder --force-clean --user --disable-cache --repo flatpak-repo flatpak-build flatpak/io.github.loveyless.ccgateway.yml
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo CC-Gateway-Linux.flatpak io.github.loveyless.ccgateway
```

4) Install and run:

```bash
flatpak install --user ./CC-Gateway-Linux.flatpak
flatpak run io.github.loveyless.ccgateway
```
...
If you prefer minimal permissions (e.g., for Flathub submission or security concerns), you can replace `--filesystem=home` in `flatpak/io.github.loveyless.ccgateway.yml` with more precise grants:

```yaml
  - --filesystem=~/.cc-gateway:create
...
If you plan to publish on Flathub or want stricter permission control, adjust the `finish-args` in `flatpak/io.github.loveyless.ccgateway.yml` accordingly.
