...
"#;
    ...
    assert!(
        config_after_takeover.contains("CC_GATEWAY_PROXY_MANAGED"),
    ...
    assert!(
        !restored_config.contains("CC_GATEWAY_PROXY_MANAGED"),
...
fn provider_service_reswitch_current_official_keeps_live_auth() {
    ...
    let auth_value: serde_json::Value =
        read_json_file(&cc_gateway_lib::get_codex_auth_path()).expect("auth.json must survive");
    assert_eq!(
        auth_value.get("OPENAI_API_KEY").and_then(|v| v.as_str()),
        Some("residue-key"),
...
fn read_codex_live_settings_tolerates_missing_auth_when_config_file_exists() {
    ...
    assert!(
        cc_gateway_lib::read_codex_live_settings().is_err(),
    ...
    let live = cc_gateway_lib::read_codex_live_settings()
...
fn sync_current_provider_for_app_keeps_live_takeover_and_updates_restore_backup() {
    ...
    let taken_over_live = json!({
        "env": {
            ...
            "ANTHROPIC_AUTH_TOKEN": "CC_GATEWAY_PROXY_MANAGED"
        }
    });
...
"#
    ...
    assert!(
        live_config.contains("CC_GATEWAY_PROXY_MANAGED"),
...
#[test]
fn switch_retired_gemini_app_is_rejected() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let _home = ensure_test_home();
    ...
    let err = ProviderService::switch(&state, AppType::Gemini, "packy-gemini")
        .expect_err("Gemini CLI app is retired");
    let message = err.to_string();
    assert!(
