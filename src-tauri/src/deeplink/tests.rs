...
impl TestHomeGuard {
    fn new() -> Self {
        ...
        let original_test_home = env::var_os("CC_GATEWAY_TEST_HOME");
...
fn test_parse_deeplink_with_notes() {
    let url = "ccgateway://v1/import?resource=provider&app=codex&name=Codex&homepage=https%3A%2F%2Fcodex.com&endpoint=https%3A%2F%2Fapi.codex.com&apiKey=key123&notes=Test%20notes";
...
fn test_parse_grokbuild_provider() {
    ...
    let url = "ccgateway://v1/import?resource=provider&app=grokbuild&name=Grok%20Relay&endpoint=https%3A%2F%2Fapi.example.com%2Fv1&apiKey=secret&model=grok-4.5";
...
#[test]
fn test_build_retired_gemini_provider_is_rejected() {
    use super::provider::build_provider_from_request;
    ...
    let err = build_provider_from_request(&AppType::Gemini, &request)
        .expect_err("Gemini CLI deeplink import is retired");
...
fn test_import_prompt_allows_space_in_base64_content() {
    ...
    let url = "ccgateway://v1/import?resource=prompt&app=codex&name=PromptPlus&content=Pj4+";
    let request = parse_deeplink_url(url).unwrap();
...
fn test_parse_mcp_apps() {
    ...
    let apps = parse_mcp_apps("grokbuild,hermes").unwrap();
...
fn test_parse_mcp_deeplink() {
    ...
    let url = format!(
        "ccgateway://v1/import?resource=mcp&apps=claude,codex&config={}&enabled=true",
        config_b64
    );
