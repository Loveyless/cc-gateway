...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/Loveyless/cc-gateway/releases/latest",
            None::<String>,
        )
...
const VALID_TOOLS: [&str; 4] = ["claude", "codex", "grok", "hermes"];
...
mod tests {
    ...
    fn resolve_launch_cwd_rejects_missing_directory() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let missing = std::env::temp_dir().join(format!("cc-gateway-missing-{unique}"));
