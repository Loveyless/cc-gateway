...
fn merge_settings_for_save(
    ...
    // Retired sync settings are intentionally not merged back into the payload.
    // They remain deserializable for old settings files but are never rewritten.
...
/// 更新链路尚未配置维护版签名和清单，因此暂时不执行下载或安装。
#[tauri::command]
pub async fn install_update_and_restart(_app: AppHandle) -> Result<bool, String> {
    log::info!("维护版更新链路未启用，跳过应用更新安装");
    Ok(false)
}

/// 维护版 updater 未配置时不执行网络检查，并返回 None。
#[tauri::command]
