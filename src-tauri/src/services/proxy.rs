...
impl ProxyService {
    ...
    async fn sync_live_to_provider(&self, app_type: &AppType) -> Result<(), String> {
        ...
        let live_config = match app_type {
            ...
            AppType::Gemini | AppType::OpenCode | AppType::OpenClaw => {
                return Err(app_type.ensure_supported().unwrap_err().to_string());
            }
    ...
    fn write_live_config_for_app(&self, app_type: &AppType, config: &Value) -> Result<(), String> {
        match app_type {
            ...
            AppType::Gemini | AppType::OpenCode | AppType::OpenClaw => {
                Err(app_type.ensure_supported().unwrap_err().to_string())
            }
    ...
    fn read_gemini_live(&self) -> Result<Value, String> {
        Err(AppType::Gemini.ensure_supported().unwrap_err().to_string())
    }

    fn write_gemini_live(&self, _config: &Value) -> Result<(), String> {
        Err(AppType::Gemini.ensure_supported().unwrap_err().to_string())
    }
...
"#,
        ...
        // Call bulk backup: must skip every remaining live-writable app
        service
            .backup_live_configs()
            .await
            .expect("bulk backup should succeed (no-op when all live are placeholders)");

        // Historical Gemini backup is left untouched; Claude/Codex skip placeholder Live.
