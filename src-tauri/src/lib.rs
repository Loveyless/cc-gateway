...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/cc-gateway.log）
            {
                ...
                log::info!("=== CC Gateway v{} started ===", env!("CARGO_PKG_VERSION"));
            }
            ...
            let db_path = app_config_dir.join("cc-gateway.db");
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 CC Gateway 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/io.github.loveyless.ccgateway/applications/cc-gateway-handler.desktop
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older CC Gateway version.\n\n\
