...
### 🌐 Upstream official website: **[ccswitch.io](https://ccswitch.io)**

> This repository is the independent `cc-gateway` maintenance fork. The upstream website, updater, and release assets are not used by this fork.
...
- **Deep Link** (`ccgateway://`) — Import providers, MCP servers, prompts, and skills via URL
...
CC Gateway supports eight tools: **Claude Code**, **Claude Desktop**, **Codex**, **Gemini CLI**, **Grok Build**, **OpenCode**, **OpenClaw**, and **Hermes**. Each tool has dedicated provider presets and configuration management.
...
The upstream CC Switch macOS builds may be code-signed and notarized by Apple. This maintenance fork currently has no published signing/notarization guarantee; verify the release assets before installing.
...
CC Gateway follows a "minimal intrusion" design principle — even if you uninstall the app, your CLI tools will continue to work normally. The system always keeps one active configuration, because deleting all configurations would make the corresponding CLI tool unusable. If you rarely use a specific CLI tool, you can hide it in Settings. To switch back to official login, see the next question.
...
- **Database**: `~/.cc-gateway/cc-gateway.db` (SQLite — providers, MCP, prompts, skills)
- **Local settings**: `~/.cc-gateway/settings.json` (device-level UI preferences)
- **Backups**: `~/.cc-gateway/backups/` (auto-rotated, keeps 10 most recent)
- **Skills**: `~/.cc-gateway/skills/` (symlinked to corresponding apps by default)
- **Skill Backups**: `~/.cc-gateway/skill-backups/` (created automatically before uninstall, keeps 20 most recent)
...
If you launch from a desktop icon, add it to the `.desktop` `Exec=` line (e.g. `env CC_GATEWAY_GDK_BACKEND=wayland /path/to/AppImage`) or set it in your session environment. The variable is generic: on tiling Wayland compositors (sway/Hyprland) where clicks don't register, try `CC_GATEWAY_GDK_BACKEND=x11` instead. Leaving it unset keeps the default behavior.
...
Download the latest `CC-Gateway-v{version}-Windows.msi` installer or `CC-Gateway-v{version}-Windows-Portable.zip` portable version from the [Releases](https://github.com/Loveyless/cc-gateway/releases) page.

### macOS Users

**Manual Download**

Download `CC-Gateway-v{version}-macOS.dmg` (recommended) or `.zip` from the [Releases](https://github.com/Loveyless/cc-gateway/releases) page.

> **Note**: This maintenance fork currently has no published macOS signing/notarization guarantee. Check the release notes and asset signatures before opening a downloaded build.

### Arch Linux Users

No CC Gateway AUR package is published yet. Use the portable AppImage or build
the package locally until a Gateway-owned package is available.

### Linux Users

Download the latest Linux build from the [Releases](https://github.com/Loveyless/cc-gateway/releases) page:

- `CC-Gateway-v{version}-Linux.deb` (Debian/Ubuntu)
- `CC-Gateway-v{version}-Linux.rpm` (Fedora/RHEL/openSUSE)
- `CC-Gateway-v{version}-Linux.AppImage` (Universal)
...
- **SSOT** (Single Source of Truth): All data stored in `~/.cc-gateway/cc-gateway.db` (SQLite)
